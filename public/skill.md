...
6. **Register.** Call:
   ...
   **The CV page carries no personal name by design.** Its title comes from `q_artifacts_tagline_one_liner` (role/one-liner) — anonymity until the candidate opts into contact via the shortlist flow. You may additionally pass `q_identity_display_name` (a free-form public name): it is NOT shown on the CV page, but is displayed on the catalog card at mcphire.com/candidates if the user opted in.
...
4. **Post vacancies.** For each opening:
   ...
   **Vacancy goes live immediately.** As of 2026-07-10 there is NO verification gate for employers: `post_vacancy` creates the vacancy with `status=active` — it is instantly visible at its public `/jobs/<slug>` URL, enters the matching loop right away (matching candidates get a Telegram push), and needs no founder review. `publish_vacancy` is kept as an idempotent no-op for backwards compatibility — calling it on an active vacancy simply returns `already_active: true`. Per-employer anti-spam caps apply (default 5 vacancies/hour, 20/day) — exceeding them returns 429. So after `post_vacancy`, tell the user the vacancy is **live** and share the `job_url`. Check anytime with `get_employer_vacancies(employer_session_token=...)` — the session token alone is enough for it and for `get_applicants(vacancy_id, employer_session_token=...)`; the legacy `employer_id` parameter is deprecated and may be omitted. Currency codes are case-insensitive (`rub` and `RUB` both work; stored upper-case).
